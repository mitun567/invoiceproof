...
export class ProofBatchService {
  ...
  async processPendingBatch(options?: { respectCollectionWindow?: boolean }) {
    const respectCollectionWindow = options?.respectCollectionWindow !== false;

    const prepared = await this.preparePendingBatch({
      respectCollectionWindow,
    });
    const submitted = await this.submitPreparedBatch(
      prepared.batchId || undefined
    );
  ...
  async processJob(jobName: string, data?: { batchId?: string }) {
    switch (jobName) {
      case "anchor.prepare":
        return this.preparePendingBatch({ respectCollectionWindow: true });
      case "anchor.submit":
        return this.submitPreparedBatch(data?.batchId);
      case "anchor.reconcile":
        return this.reconcilePendingReceipts();
      case "anchor.backlog-scan":
        return this.processPendingBatch({ respectCollectionWindow: false });
      case "anchor.windowed-submit":
      case "anchor.tick":
      default:
        return this.processPendingBatch({ respectCollectionWindow: true });
    }
  }

  async preparePendingBatch(options?: { respectCollectionWindow?: boolean }) {
    const batchSize = envInt("ANCHOR_BATCH_SIZE", 100, 1);
    const collectionWindowMs = envInt("ANCHOR_COLLECTION_WINDOW_MS", 5000, 0);
    const respectCollectionWindow =
      options?.respectCollectionWindow !== false && collectionWindowMs > 0;
    ...
    if (respectCollectionWindow) {
      where.createdAt = {
        lte: new Date(Date.now() - collectionWindowMs),
      };
    }
    ...
    if (!invoices.length) {
      this.logger.log(
        respectCollectionWindow
          ? "No pending invoices are old enough for the current collection window."
          : "No pending invoices found for backlog batch preparation."
      );
      return {
        prepared: 0,
        batchId: null,
        respectedCollectionWindow: respectCollectionWindow,
      };
    }
    ...
    this.logger.log(
      `Prepared anchor batch ${batch.id} for ${invoices.length} invoice(s). respectCollectionWindow=${respectCollectionWindow}`
    );
  ...
  async submitPreparedBatch(batchId?: string) {
    ...
    if (!batch.invoices.length) {
      await this.prisma.client.anchorBatch.update({
        ...
        data: {
          status: "anchor_failed_final",
          failureReason: "Prepared batch has no invoices assigned.",
        },
      });
      ...
      return {
        submitted: 0,
        batchId: batch.id,
        error: "Prepared batch has no invoices assigned.",
      };
    }
    ...
    try {
      ...
      await this.prisma.client.$transaction([
        this.prisma.client.anchorBatch.update({
          where: { id: batch.id },
          data: {
            status: "anchor_send_failed_retryable",
            failureReason: message,
            retryCount: { increment: 1 },
          },
        }),
        ...
        this.prisma.client.auditLog.createMany({
          data: batch.invoices.map((invoice: any) => ({
            orgId: invoice.orgId,
            action: "proof.anchor_send_failed",
            entityType: "InvoiceRecord",
            entityId: invoice.id,
            metadataJson: {
              batchId: batch.id,
  ...
  async reconcilePendingReceipts() {
    const proofUpsertChunkSize = envInt("PROOF_UPSERT_CHUNK_SIZE", 20, 1);
    const maxBatchesPerPass = envInt(
      "ANCHOR_RECONCILE_BATCHES_PER_PASS",
      10,
      1
    );
    const confirmations = envInt("ANCHOR_CONFIRMATIONS", 1, 1);
    ...
    for (const batch of batches) {
      ...
      try {
        ...
        const persistedMerkleRoot = batch.merkleRoot.startsWith("0x")
          ? batch.merkleRoot
          : `0x${batch.merkleRoot}`;
        const anchoredAt = receiptResult.anchoredAt || new Date();
        ...
        const merkle = buildMerkleTree(
          batch.invoices.map((invoice: any) => invoice.recordHash)
        );

        for (
          let start = 0;
          start < batch.invoices.length;
          start += proofUpsertChunkSize
        ) {
          const chunk = batch.invoices.slice(
            start,
            start + proofUpsertChunkSize
          );
